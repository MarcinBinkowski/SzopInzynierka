  ...
  const removeFromWishlistMutation = useCatalogWishlistDestroy({
    mutation: {
      onSuccess: (data, variables) => {
        ...
        if (productId) {
          queryClient.invalidateQueries({ queryKey: getCatalogWishlistCheckRetrieveQueryKey(productId) });
        }
        queryClient.invalidateQueries({ queryKey: getCatalogWishlistListQueryKey() });
        refetchWishlist();
      },
      onError: () => {
  ...
  const handleRemoveFromWishlist = useCallback((itemId: number) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Remove', style: 'destructive', onPress: () => removeFromWishlistMutation.mutate({ id: itemId.toString() }) },
      ]
    );
  }, [removeFromWishlistMutation]);

  const handleProductPress = useCallback((productId: number) => {
    router.push({ pathname: '/product/[id]', params: { id: String(productId), from: 'wishlist' } });
  }, []);
