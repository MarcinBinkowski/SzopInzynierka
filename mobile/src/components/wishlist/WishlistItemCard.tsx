...
export function WishlistItemCard({ item, onPress, onRemove, isRemoving }: WishlistItemCardProps) {
  const theme = useTheme();
  return (
    <Card key={item.id} style={styles.wishlistCard}>
      <TouchableOpacity onPress={() => onPress(item.product.id)} activeOpacity={0.7}>
        <Card.Content style={styles.cardContent}>
          <View style={styles.cardHeader}>
            <View style={styles.imageContainer}>
              {item.product.primary_image ? (
                <Image source={{ uri: item.product.primary_image }} style={styles.productImage} />
              ) : (
...
const styles = StyleSheet.create({
  ...
  imageContainer: { width: 80, height: 80, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  productImage: { width: 80, height: 80, borderRadius: 8 },
  placeholderImage: { width: 80, height: 80, borderRadius: 8, backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center' },
