...
export default function PriceBadge({ product }: { product: ProductList | ProductDetail }) {
  const theme = useTheme();
  const { is_on_sale, original_price, current_price } = product;

  if (!is_on_sale) {
    return <Text variant="titleMedium" style={{ color: theme.colors.onSurfaceVariant, marginTop: 8 }}>
        ${current_price}
    </Text>
  }
