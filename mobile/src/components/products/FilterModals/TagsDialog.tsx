...
export function TagsDialog({
  ...
  // Fetch tags data with pagination and search
  const {
    data: tagsPages,
    isLoading,
    fetchNextPage,
  ...
  const renderTag = ({ item: tag }: { item: Tag }) => (
    <List.Item
      ...
      left={(props) => (
        <List.Icon 
          {...props} 
          icon={draftTags.includes(tag.slug) ? 'check-circle' : 'radiobox-blank'} 
        />
      )}
      onPress={() => handleTagToggle(tag.slug)}
      style={{
        backgroundColor: draftTags.includes(tag.slug) ? theme.colors.primaryContainer : undefined,
      }}
    />
  );
