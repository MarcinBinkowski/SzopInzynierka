...
class InvoiceTemplateService:
    ...
    def generate_pdf(cls, html_content: str, filename: str = None) -> bytes:
        ...
        try:
            ...
            # Configure pdfkit options with more robust settings
            options = {
                "page-size": "A4",
                "margin-top": "0.75in",
                "margin-right": "0.75in",
    ...
    def _build_context(cls, order) -> Dict[str, Any]:
        ...
        logger = logging.getLogger(__name__)
        logger.info(f"Order {order.order_number} has {order_items.count()} items")

        # Pass whole objects and let template validation handle allowed properties
        context = {
            "order": order,
