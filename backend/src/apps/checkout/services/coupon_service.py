...
class CouponService:
    ...
    def validate_coupon(coupon: Coupon, user: User, cart: Cart) -> tuple[bool, str]:
        ...
        if now < coupon.valid_from:
            return False, f"Coupon '{coupon.code}' is not valid yet. Valid from {coupon.valid_from.strftime('%Y-%m-%d')}"
        
        if now > coupon.valid_until:
            return False, f"Coupon '{coupon.code}' has expired. Valid until {coupon.valid_until.strftime('%Y-%m-%d')}"
        ...
        if user_redemptions >= coupon.max_uses_per_user:
            return False, f"You have already used coupon '{coupon.code}' {user_redemptions} out of maximum {coupon.max_uses_per_user} uses."
