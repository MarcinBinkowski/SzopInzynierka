...
class DashboardAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        methods = list(qs.values("shipping_method__name").annotate(count=Count("id")).order_by("-count"))
        methods_entries = [{"name": m["shipping_method__name"] or "Unknown", "count": m["count"]} for m in methods]
        methods_total = sum(m["count"] for m in methods_entries)

        products = list(items_qs.values("product__name").annotate(qty=Sum("quantity"), revenue=Sum("total_price")).order_by("-qty"))
        products_entries = [{"name": p["product__name"] or "Unknown", "qty": int(p["qty"] or 0), "revenue": float(p["revenue"] or 0)} for p in products]
        total_qty = sum(p["qty"] for p in products_entries)

        manufacturers = list(items_qs.values("product__manufacturer__name").annotate(qty=Sum("quantity")).order_by("-qty"))
        manufacturers_entries = [{"name": m["product__manufacturer__name"] or "Unknown", "qty": int(m["qty"] or 0)} for m in manufacturers]

        tags = list(items_qs.filter(product__tags__isnull=False).values("product__tags__name").annotate(qty=Sum("quantity")).order_by("-qty"))
        tags_entries = [{"name": t["product__tags__name"] or "Unknown", "qty": int(t["qty"] or 0)} for t in tags]
...
class OrdersExportCsvView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: OpenApiResponse(response=OpenApiTypes.BINARY, description='CSV file (text/csv)')}
    )
    def get(self, request):
        period = request.query_params.get("period", "7d")
        now = timezone.now()
        ...
        for o in qs.iterator():
            shipping_method_name = getattr(o.shipping_method, 'name', '') if o.shipping_method else ''
            coupon_code = getattr(o.applied_coupon, 'code', '') if o.applied_coupon else ''
            ...
            if shipping_address is not None:
                ...
                buyer_display_name = getattr(profile, 'display_name', '') if profile else ''
