...
export type UseListHook<T, Q = any> = (params: { query: Q }) => ListHookResult<T>;
...
export function GenericReadOnlyListPage<T extends MRT_RowData, Q = any>({
  ...
  // Support array or {results, count}
  const items: T[] = Array.isArray(data)
    ? (data as T[])
    : (data as Paginated<T>)?.results ?? [];
  const rowCount = enablePagination
    ? (Array.isArray(data) ? (data as T[]).length : (data as Paginated<T>)?.count ?? items.length)
    : items.length;
