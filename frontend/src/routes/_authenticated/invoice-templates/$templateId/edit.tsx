...
type InvoiceTemplateFormData = z.infer<typeof checkoutInvoiceTemplatesUpdateBody>

function EditInvoiceTemplatePage() {
  const navigate = useNavigate()
  const { templateId } = useParams({ from: "/_authenticated/invoice-templates/$templateId/edit" })
  const updateMutation = useCheckoutInvoiceTemplatesUpdate()
  
  const { data: template, isLoading, error } = useCheckoutInvoiceTemplatesRetrieve(Number(templateId))
...
export const Route = createFileRoute('/_authenticated/invoice-templates/$templateId/edit')({
