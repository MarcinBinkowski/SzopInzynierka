...
import { useCatalogDeliveriesRetrieve, useCatalogDeliveriesUpdate } from "@/api/generated/shop/catalog/catalog"
...
function EditDeliveryPage() {
  ...
  const { deliveryId } = useParams({ from: '/_authenticated/catalog/deliveries/$deliveryId/edit' })
  const updateMutation = useCatalogDeliveriesUpdate()
  const { data: delivery, isLoading, error } = useCatalogDeliveriesRetrieve(parseInt(deliveryId), { query: { enabled: !!deliveryId }})

  const handleSubmit = async (formData: ProductDeliveryFormData) => {
    try {
      const validatedData = catalogDeliveriesUpdateBody.parse(formData)
      await updateMutation.mutateAsync({ id: parseInt(deliveryId), data: validatedData })
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex items-center space-x-2">
          <Spinner size="lg" />
          <span className="text-sm text-muted-foreground">Loading delivery...</span>
        </div>
      </div>
    )
  }
...
export const Route = createFileRoute('/_authenticated/catalog/deliveries/$deliveryId/edit')({
