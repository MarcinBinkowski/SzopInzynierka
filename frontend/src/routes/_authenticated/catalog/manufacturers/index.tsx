...
import { useCatalogManufacturersList, useCatalogManufacturersDestroy } from "@/api/generated/shop/catalog/catalog"
...
function ManufacturersPage() {
  ...
  const { data, isLoading, refetch } = useCatalogManufacturersList(apiParams)
  ...
  const rowActions = useStandardRowActions({
    editRoute: row => `/catalog/manufacturers/${row.original?.id || row.id}/edit`,
    onDelete: async row => {
      await deleteMutation.mutateAsync({ id: row.original?.id || row.id })
      refetch()
    },
    deleteConfirmMessage: row => `Delete manufacturer "${row.original?.name || row.name}"?`,
    deleteSuccessMessage: "Manufacturer deleted"
  })
