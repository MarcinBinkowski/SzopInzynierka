...
import { useCheckoutCouriersRetrieve, useCheckoutCouriersUpdate } from "@/api/generated/shop/checkout/checkout"
...
function EditCourierPage() {
  ...
  const { courierId } = useParams({ from: '/_authenticated/checkout/couriers/$courierId/edit' })
  const updateMutation = useCheckoutCouriersUpdate()
  const { data: courier, isLoading, error } = useCheckoutCouriersRetrieve(parseInt(courierId), { query: { enabled: !!courierId }})

  const handleSubmit = async (formData: CourierFormData) => {
    try {
      const validatedData = checkoutCouriersUpdateBody.parse(formData)
      await updateMutation.mutateAsync({ id: parseInt(courierId), data: validatedData })
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex items-center space-x-2">
          <Spinner size="lg" />
          <span className="text-sm text-muted-foreground">Loading courier...</span>
        </div>
      </div>
    )
  }
...
export const Route = createFileRoute('/_authenticated/checkout/couriers/$courierId/edit')({
