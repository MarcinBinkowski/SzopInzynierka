...
import { useCheckoutShippingMethodsList, useCheckoutShippingMethodsDestroy } from '@/api/generated/shop/checkout/checkout'
...
export const Route = createFileRoute('/_authenticated/checkout/shipping-methods/')({
...
function ShippingMethodsPage() {
  ...
  return (
    <CrudListPage
      ...
      onEdit={(row) => navigate({ to: `/checkout/shipping-methods/${row.id}/edit` })}
      onDelete={async (row) => {
