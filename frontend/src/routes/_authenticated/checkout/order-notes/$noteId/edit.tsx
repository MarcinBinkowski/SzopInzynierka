...
import { useCheckoutOrderNotesRetrieve, useCheckoutOrderNotesUpdate } from "@/api/generated/shop/checkout/checkout"
...
function EditOrderNotePage() {
  ...
  const { noteId } = useParams({ from: '/_authenticated/checkout/order-notes/$noteId/edit' })
  const updateMutation = useCheckoutOrderNotesUpdate()
  const { data: note, isLoading, error } = useCheckoutOrderNotesRetrieve(parseInt(noteId), { query: { enabled: !!noteId }})

  const handleSubmit = async (formData: OrderNoteFormData) => {
    try {
      const validatedData = checkoutOrderNotesUpdateBody.parse(formData)
      await updateMutation.mutateAsync({ id: parseInt(noteId), data: validatedData })
...
export const Route = createFileRoute('/_authenticated/checkout/order-notes/$noteId/edit')({
