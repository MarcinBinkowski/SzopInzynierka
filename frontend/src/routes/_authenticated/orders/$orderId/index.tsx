    ...
    <div className="container mx-auto py-6 space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="text-lg">{new Date(order.created_at).toLocaleDateString()}</div>
            </div>
            <div>
              ...
              {order.items.map((item: any) => (
                <div key={item.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="font-semibold">{item.product?.name || `Product #${item.product?.id}`}</div>
                      <div className="text-sm text-muted-foreground">Quantity: {item.quantity}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-semibold">${item.unit_price}</div>
                    <div className="text-sm text-muted-foreground">Total: ${item.total_price}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div>
              ...
              <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                {order.shipping_address ? (
                  <div>
                    <div className="font-medium">{order.shipping_address.full_address}</div>
                    {order.shipping_address.label && (
                      <div className="text-sm text-muted-foreground">Label: {order.shipping_address.label}</div>
                    )}
                  </div>
                ) : (
            ...
            <div>
              ...
              <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                {order.shipping_method ? (
                  <div>
                    <div className="font-medium">{order.shipping_method.name}</div>
                    <div className="text-sm text-muted-foreground">Cost: ${order.shipping_cost}</div>
                  </div>
                ) : (
                  <div className="text-muted-foreground">No shipping method</div>
                )}
              </div>
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="text-sm font-medium text-muted-foreground">Applied Coupon</label>
              <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                {order.applied_coupon ? (
                  <div>
                    <div className="font-medium">{order.applied_coupon.code}</div>
                    <div className="text-sm text-muted-foreground">Discount: ${order.coupon_discount}</div>
                  </div>
                ) : (
